...
impl RosMsg {
    ...
    fn to_string(&self) -> String {
        ...
        buf.push_str(&format!("struct {} {{", &self.name));
...
fn fmt_file(path: &PathBuf) -> Result<()> {
    ...
    fmt_cmd.arg("--quiet").arg(path).output()?;
    Ok(())
}
...
fn write(
    ...
    writer.write("mod msgs {".as_bytes())?;

    for package in mods.values() {
        writer.write(format!("mod {} {{", package).as_bytes())?;
        ...
        for (msg_path, msg) in msgs.iter() {
            ...
            match mods.get(parent_path.to_str().unwrap_or("")) {
                Some(cur_package) => {
                    ...
                    writer.write("#[derive(Clone, Copy, Debug, Deserialize, Eq)]".as_bytes())?;
                    writer.write(msg.to_string().as_bytes())?;
                    write!(writer, "impl Msg for {} {{}}", msg.name)?;
                }
                None => println!(
                    "{}",
                    format!("WARN: missing package for {}", &msg_path.to_string_lossy())
                ),
            };
        }
...
fn get_mods_and_msgs(
    ...
    for entry in WalkDir::new(input_path).into_iter() {
        ...
        } else if abs_path.extension().unwrap().to_string_lossy() == "msg" {
...
fn main() -> Result<()> {
    ...
    write(&opts.output_path, mods, msgs)?;
